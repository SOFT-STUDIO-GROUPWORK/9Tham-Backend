...
public class ArticleRepository : IArticleRepository
{
    ...
    public async Task<ArticlePaginationModel> GetArticlesPaginated(int page,float perPage)
    {
        ...
        var articles = await _context.Articles.Skip((page - 1) * (int) perPage).Take((int)perPage).Include(a=>a.ArticleTags).ThenInclude(at=>at.Tag).Where(e => e.Visible==false).ToListAsync();
    ...
    public async Task<ArticlePaginationModel> GetReverseArticlesPaginated(int page,float perPage)
    {
        var pageCount = Math.Ceiling(_context.Articles.Count() / perPage);
        if (pageCount == 0) pageCount = 1;
        var reverseDB = await _context.Articles.Include(a=>a.ArticleTags).ThenInclude(at=>at.Tag).Where(e => e.Visible==false).ToListAsync();
    ...
    {
        var qureyWhere = await _context.Articles.Include(a=>a.Blogger).Include(a=>a.ArticleTags).ThenInclude(at=>at.Tag).Where(e => e.Title.Contains(search) || e.Content.Contains(search)).Where(e => e.Visible==false).ToListAsync();
    ...
    {
        var reverseDB = await _context.Articles.Include(a=>a.Blogger).Include(a=>a.ArticleTags).ThenInclude(at=>at.Tag).Where(e => e.Title.Contains(search) || e.Content.Contains(search)).Where(e => e.Visible==false).ToListAsync();
        reverseDB = Enumerable.Reverse(reverseDB).ToList();
